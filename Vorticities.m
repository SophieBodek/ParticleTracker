function [vort,x,y,t,tr]=Vorticities(vtracks,framerange,noisy)
% Usage: [vort,x,y,t,tr]=Vorticities(vtracks,[framerange],[noisy])
% Working from the velocity tracks in "vtracks", Vorticities plots the
% vorticity of each particle in frames "framerange". Vorticity values are 
% returned in the vector "vort"; corresponding positions, times, and 
% track indices are returned in "x", "y", and "t". If noisy==0, no plot 
...
if nargin<1
    error(['Usage: [vort,x,y,t,tr] = ' mfilename ...
...
for ii=1:numel(tlist)
    ...
    [junk,uy]=pdegrad([x(ind)' ; y(ind)'],tri',u(ind));
    [vx,junk]=pdegrad([x(ind)' ; y(ind)'],tri',v(ind));
    vort(ind)=pdeprtni([x(ind)' ; y(ind)'],tri',vx-uy);
end
...
if noisy
    figure;
    scatter(x,y,[],vort,'o','filled','displayname','vorticity');
    if which('redblue')
        set(gcf,'colormap',redblue);
    end
    colorbar
    set(gca,'dataaspectratio',[1 1 1], ...
        'clim',prctile(abs(vort),100-satgoal)*[-1 1]);
    axis tight
    if framerange(1)==framerange(2)
        title(['vorticity at ' num2str(numel(x)) ' particles in frame ' ...
        ...
        title(['vorticity at ' num2str(numel(x)) ' particles in frames ' ...
